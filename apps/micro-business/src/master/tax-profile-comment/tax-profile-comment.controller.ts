...
import {
  CreateTaxProfileCommentSchema,
  UpdateTaxProfileCommentSchema,
  AttachmentSchema,
...
export class TaxProfileCommentController {
  private readonly logger: BackendLogger = new BackendLogger(
    TaxProfileCommentController.name,
  );

  constructor(
    private readonly taxProfileCommentService: TaxProfileCommentService,
  ) {}
   ...
   */
  @MessagePattern({
    cmd: 'tax-profile-comment.find-by-id',
    service: 'tax-profile-comment',
  })
  async findById(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    await this.taxProfileCommentService.initializePrismaService(
      bu_code,
      user_id,
    );
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.taxProfileCommentService.findById(id),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'tax-profile-comment.find-all-by-tax-profile-id',
    service: 'tax-profile-comment',
  })
  async findAllByTaxProfileId(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    this.logger.debug(
      { function: 'findAllByTaxProfileId', payload },
      TaxProfileCommentController.name,
    );

    const { tax_profile_id, user_id, bu_code, paginate } = payload;
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.taxProfileCommentService.findAllByTaxProfileId(
        tax_profile_id,
        paginate,
      ),
    );
   ...
   */
  @MessagePattern({
    cmd: 'tax-profile-comment.create',
    service: 'tax-profile-comment',
  })
  async create(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const parsedData = CreateTaxProfileCommentSchema.parse(data);

    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.taxProfileCommentService.create(parsedData),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'tax-profile-comment.update',
    service: 'tax-profile-comment',
  })
  async update(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const parsedData = UpdateTaxProfileCommentSchema.parse(data);

    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.taxProfileCommentService.update(id, parsedData),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'tax-profile-comment.delete',
    service: 'tax-profile-comment',
  })
  async delete(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.taxProfileCommentService.delete(id),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'tax-profile-comment.add-attachment',
    service: 'tax-profile-comment',
  })
  async addAttachment(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.taxProfileCommentService.addAttachment(id, parsedAttachment),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'tax-profile-comment.remove-attachment',
    service: 'tax-profile-comment',
  })
  async removeAttachment(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.taxProfileCommentService.removeAttachment(id, fileToken),
    );
  }
}
