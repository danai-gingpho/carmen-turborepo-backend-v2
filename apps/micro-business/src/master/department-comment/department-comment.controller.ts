...
import {
  CreateDepartmentCommentSchema,
  UpdateDepartmentCommentSchema,
  AttachmentSchema,
} from './dto/department-comment.dto';
...
export class DepartmentCommentController {
  private readonly logger: BackendLogger = new BackendLogger(
    DepartmentCommentController.name,
  );

  constructor(
    private readonly departmentCommentService: DepartmentCommentService,
  ) {}
   ...
   */
  @MessagePattern({
    cmd: 'department-comment.find-by-id',
    service: 'department-comment',
  })
  async findById(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    await this.departmentCommentService.initializePrismaService(
      bu_code,
      user_id,
    );
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.departmentCommentService.findById(id),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'department-comment.find-all-by-department-id',
    service: 'department-comment',
  })
  async findAllByDepartmentId(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    this.logger.debug(
      { function: 'findAllByDepartmentId', payload },
      DepartmentCommentController.name,
    );

    const { department_id, user_id, bu_code, paginate } = payload;
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.departmentCommentService.findAllByDepartmentId(
        department_id,
        paginate,
      ),
    );
  ...
  async create(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const parsedData = CreateDepartmentCommentSchema.parse(data);

    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.departmentCommentService.create(parsedData),
    );
  }
  ...
  async update(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const parsedData = UpdateDepartmentCommentSchema.parse(data);

    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.departmentCommentService.update(id, parsedData),
    );
  }
  ...
  async delete(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.departmentCommentService.delete(id),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'department-comment.add-attachment',
    service: 'department-comment',
  })
  async addAttachment(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.departmentCommentService.addAttachment(id, parsedAttachment),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'department-comment.remove-attachment',
    service: 'department-comment',
  })
  async removeAttachment(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.departmentCommentService.removeAttachment(id, fileToken),
    );
  }
}
