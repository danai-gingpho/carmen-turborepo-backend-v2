...
import {
  CreateUnitCommentSchema,
  UpdateUnitCommentSchema,
  AttachmentSchema,
} from './dto/unit-comment.dto';
...
export class UnitCommentController {
  private readonly logger: BackendLogger = new BackendLogger(
    UnitCommentController.name,
  );

  constructor(
    private readonly unitCommentService: UnitCommentService,
  ) {}
   ...
   */
  @MessagePattern({
    cmd: 'unit-comment.find-by-id',
    service: 'unit-comment',
  })
  async findById(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    await this.unitCommentService.initializePrismaService(
      bu_code,
      user_id,
    );
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.unitCommentService.findById(id),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'unit-comment.find-all-by-unit-id',
    service: 'unit-comment',
  })
  async findAllByUnitId(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    this.logger.debug(
      { function: 'findAllByUnitId', payload },
      UnitCommentController.name,
    );

    const { unit_id, user_id, bu_code, paginate } = payload;
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.unitCommentService.findAllByUnitId(
        unit_id,
        paginate,
      ),
    );
  ...
  async create(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const parsedData = CreateUnitCommentSchema.parse(data);

    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.unitCommentService.create(parsedData),
    );
  }
  ...
  async update(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const parsedData = UpdateUnitCommentSchema.parse(data);

    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.unitCommentService.update(id, parsedData),
    );
  }
  ...
  async delete(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.unitCommentService.delete(id),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'unit-comment.add-attachment',
    service: 'unit-comment',
  })
  async addAttachment(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.unitCommentService.addAttachment(id, parsedAttachment),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'unit-comment.remove-attachment',
    service: 'unit-comment',
  })
  async removeAttachment(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.unitCommentService.removeAttachment(id, fileToken),
    );
  }
}
