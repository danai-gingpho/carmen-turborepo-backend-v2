...
import {
  CreatePricelistCommentSchema,
  UpdatePricelistCommentSchema,
  AttachmentSchema,
} from './dto/pricelist-comment.dto';
...
export class PricelistCommentController {
  private readonly logger: BackendLogger = new BackendLogger(
    PricelistCommentController.name,
  );

  constructor(
    private readonly pricelistCommentService: PricelistCommentService,
  ) {}
   ...
   */
  @MessagePattern({
    cmd: 'pricelist-comment.find-by-id',
    service: 'pricelist-comment',
  })
  async findById(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    await this.pricelistCommentService.initializePrismaService(
      bu_code,
      user_id,
    );
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.pricelistCommentService.findById(id),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'pricelist-comment.find-all-by-pricelist-id',
    service: 'pricelist-comment',
  })
  async findAllByPricelistId(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    this.logger.debug(
      { function: 'findAllByPricelistId', payload },
      PricelistCommentController.name,
    );

    const { pricelist_id, user_id, bu_code, paginate } = payload;
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.pricelistCommentService.findAllByPricelistId(
        pricelist_id,
        paginate,
      ),
    );
  ...
  async create(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const parsedData = CreatePricelistCommentSchema.parse(data);

    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.pricelistCommentService.create(parsedData),
    );
  }
  ...
  async update(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const parsedData = UpdatePricelistCommentSchema.parse(data);

    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.pricelistCommentService.update(id, parsedData),
    );
  }
  ...
  async delete(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.pricelistCommentService.delete(id),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'pricelist-comment.add-attachment',
    service: 'pricelist-comment',
  })
  async addAttachment(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.pricelistCommentService.addAttachment(id, parsedAttachment),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'pricelist-comment.remove-attachment',
    service: 'pricelist-comment',
  })
  async removeAttachment(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.pricelistCommentService.removeAttachment(id, fileToken),
    );
  }
}
