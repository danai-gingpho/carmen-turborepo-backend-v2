...
import {
  CreatePricelistTemplateComment,
  UpdatePricelistTemplateComment,
  Attachment,
...
export class PricelistTemplateCommentService {
  ...
  async findById(id: string): Promise<any> {
    ...
    const comment = await this.prismaService.tb_pricelist_template_comment.findFirst({
      where: {
        id: id,
        deleted_at: null,
      },
   ...
   * @param pricelistTemplateId - Purchase request ID / ID ของใบขอซื้อ
   * @param paginate - Pagination parameters / พารามิเตอร์การแบ่งหน้า
   * @returns Paginated list of comments with user profiles / รายการความคิดเห็นพร้อมโปรไฟล์ผู้ใช้ที่แบ่งหน้าแล้ว
   */
  async findAllByPricelistTemplateId(pricelistTemplateId: string, paginate: IPaginate): Promise<any> {
    this.logger.debug(
      {
        function: "findAllByPricelistTemplateId",
        pricelistTemplateId,
        user_id: this.userId,
        tenant_id: this.bu_code,
        paginate,
    ...
    const whereQry = {
      ...q.where(),
      pricelist_template_id: pricelistTemplateId,
      deleted_at: null,
    };

    const comments = await this.prismaService.tb_pricelist_template_comment.findMany({
      where: whereQry,
      orderBy: q.orderBy().length > 0 ? q.orderBy() : { created_at: "desc" },
      ...pagination,
    });

    const total = await this.prismaService.tb_pricelist_template_comment.count({
      where: whereQry,
    });
   ...
   */
  async create(createDto: CreatePricelistTemplateComment): Promise<any> {
    this.logger.debug(
      {
        function: "create",
        createDto,
    ...
    const pricelistTemplate = await this.prismaService.tb_pricelist_template.findFirst({
      where: {
        id: createDto.pricelist_template_id,
      },
    });
    ...
    const comment = await this.prismaService.tb_pricelist_template_comment.create({
      data: {
        pricelist_template_id: createDto.pricelist_template_id,
   ...
   */
  async update(id: string, updateDto: UpdatePricelistTemplateComment): Promise<any> {
    this.logger.debug(
      {
        function: "update",
        id,
    ...
    const existingComment = await this.prismaService.tb_pricelist_template_comment.findFirst({
      where: {
        id: id,
        deleted_at: null,
      },
    ...
    const comment = await this.prismaService.tb_pricelist_template_comment.update({
      where: { id: id },
      data: updateData,
    });
  ...
  async delete(id: string): Promise<any> {
    ...
    // Soft delete
    await this.prismaService.tb_pricelist_template_comment.update({
      where: { id: id },
      data: {
        deleted_at: new Date(),
