...
import {
  CreateVendorBusinessTypeComment,
  UpdateVendorBusinessTypeComment,
  Attachment,
...
export class VendorBusinessTypeCommentService {
  ...
  async findById(id: string): Promise<any> {
    ...
    const comment = await this.prismaService.tb_vendor_business_type_comment.findFirst({
      where: {
        id: id,
        deleted_at: null,
      },
   ...
   * @param vendorBusinessTypeId - Purchase request ID / ID ของใบขอซื้อ
   * @param paginate - Pagination parameters / พารามิเตอร์การแบ่งหน้า
   * @returns Paginated list of comments with user profiles / รายการความคิดเห็นพร้อมโปรไฟล์ผู้ใช้ที่แบ่งหน้าแล้ว
   */
  async findAllByVendorBusinessTypeId(vendorBusinessTypeId: string, paginate: IPaginate): Promise<any> {
    this.logger.debug(
      {
        function: "findAllByVendorBusinessTypeId",
    ...
    const whereQry = {
      ...q.where(),
      vendor_business_type_id: vendorBusinessTypeId,
      deleted_at: null,
    };

    const comments = await this.prismaService.tb_vendor_business_type_comment.findMany({
      where: whereQry,
      orderBy: q.orderBy().length > 0 ? q.orderBy() : { created_at: "desc" },
      ...pagination,
    });

    const total = await this.prismaService.tb_vendor_business_type_comment.count({
      where: whereQry,
    });
   ...
   */
  async create(createDto: CreateVendorBusinessTypeComment): Promise<any> {
    this.logger.debug(
      {
        function: "create",
        createDto,
    ...
    const vendorBusinessType = await this.prismaService.tb_vendor_business_type.findFirst({
      where: {
        id: createDto.vendor_business_type_id,
      },
    });
    ...
    const comment = await this.prismaService.tb_vendor_business_type_comment.create({
      data: {
        vendor_business_type_id: createDto.vendor_business_type_id,
   ...
   */
  async update(id: string, updateDto: UpdateVendorBusinessTypeComment): Promise<any> {
    this.logger.debug(
      {
        function: "update",
        id,
    ...
    const existingComment = await this.prismaService.tb_vendor_business_type_comment.findFirst({
      where: {
        id: id,
        deleted_at: null,
      },
    ...
    const comment = await this.prismaService.tb_vendor_business_type_comment.update({
      where: { id: id },
      data: updateData,
    });
  ...
  async delete(id: string): Promise<any> {
    ...
    // Soft delete
    await this.prismaService.tb_vendor_business_type_comment.update({
      where: { id: id },
      data: {
        deleted_at: new Date(),
