...
import {
  CreateCreditTermComment,
  UpdateCreditTermComment,
  Attachment,
...
export class CreditTermCommentService {
  ...
  async findById(id: string): Promise<any> {
    ...
    const comment = await this.prismaService.tb_credit_term_comment.findFirst({
      where: {
        id: id,
        deleted_at: null,
      },
   ...
   * @param creditTermId - Purchase request ID / ID ของใบขอซื้อ
   * @param paginate - Pagination parameters / พารามิเตอร์การแบ่งหน้า
   * @returns Paginated list of comments with user profiles / รายการความคิดเห็นพร้อมโปรไฟล์ผู้ใช้ที่แบ่งหน้าแล้ว
   */
  async findAllByCreditTermId(creditTermId: string, paginate: IPaginate): Promise<any> {
    this.logger.debug(
      {
        function: "findAllByCreditTermId",
        creditTermId,
        user_id: this.userId,
        tenant_id: this.bu_code,
        paginate,
    ...
    const comments = await this.prismaService.tb_credit_term_comment.findMany({
      where: whereQry,
      orderBy: q.orderBy().length > 0 ? q.orderBy() : { created_at: "desc" },
      ...pagination,
    });

    const total = await this.prismaService.tb_credit_term_comment.count({
      where: whereQry,
    });
   ...
   */
  async create(createDto: CreateCreditTermComment): Promise<any> {
    this.logger.debug(
      {
        function: "create",
        createDto,
    ...
    const comment = await this.prismaService.tb_credit_term_comment.create({
      data: {
        credit_term_id: createDto.credit_term_id,
   ...
   */
  async update(id: string, updateDto: UpdateCreditTermComment): Promise<any> {
    this.logger.debug(
      {
        function: "update",
        id,
    ...
    const existingComment = await this.prismaService.tb_credit_term_comment.findFirst({
      where: {
        id: id,
        deleted_at: null,
      },
    ...
    const comment = await this.prismaService.tb_credit_term_comment.update({
      where: { id: id },
      data: updateData,
    });
  ...
  async delete(id: string): Promise<any> {
    ...
    // Soft delete
    await this.prismaService.tb_credit_term_comment.update({
      where: { id: id },
      data: {
        deleted_at: new Date(),
