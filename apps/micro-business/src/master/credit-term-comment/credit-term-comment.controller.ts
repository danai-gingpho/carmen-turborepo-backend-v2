...
import {
  CreateCreditTermCommentSchema,
  UpdateCreditTermCommentSchema,
  AttachmentSchema,
} from './dto/credit-term-comment.dto';
...
export class CreditTermCommentController {
  private readonly logger: BackendLogger = new BackendLogger(
    CreditTermCommentController.name,
  );

  constructor(
    private readonly creditTermCommentService: CreditTermCommentService,
  ) {}
   ...
   */
  @MessagePattern({
    cmd: 'credit-term-comment.find-by-id',
    service: 'credit-term-comment',
  })
  async findById(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    await this.creditTermCommentService.initializePrismaService(
      bu_code,
      user_id,
    );
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.creditTermCommentService.findById(id),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'credit-term-comment.find-all-by-credit-term-id',
    service: 'credit-term-comment',
  })
  async findAllByCreditTermId(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    this.logger.debug(
      { function: 'findAllByCreditTermId', payload },
      CreditTermCommentController.name,
    );

    const { credit_term_id, user_id, bu_code, paginate } = payload;
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.creditTermCommentService.findAllByCreditTermId(
        credit_term_id,
        paginate,
      ),
    );
   ...
   */
  @MessagePattern({
    cmd: 'credit-term-comment.create',
    service: 'credit-term-comment',
  })
  async create(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const parsedData = CreateCreditTermCommentSchema.parse(data);

    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.creditTermCommentService.create(parsedData),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'credit-term-comment.update',
    service: 'credit-term-comment',
  })
  async update(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const parsedData = UpdateCreditTermCommentSchema.parse(data);

    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.creditTermCommentService.update(id, parsedData),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'credit-term-comment.delete',
    service: 'credit-term-comment',
  })
  async delete(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.creditTermCommentService.delete(id),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'credit-term-comment.add-attachment',
    service: 'credit-term-comment',
  })
  async addAttachment(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.creditTermCommentService.addAttachment(id, parsedAttachment),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'credit-term-comment.remove-attachment',
    service: 'credit-term-comment',
  })
  async removeAttachment(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.creditTermCommentService.removeAttachment(id, fileToken),
    );
  }
}
