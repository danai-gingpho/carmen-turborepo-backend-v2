...
import {
  CreateProductSubCategoryComment,
  UpdateProductSubCategoryComment,
  Attachment,
...
export class ProductSubCategoryCommentService {
  ...
  async findById(id: string): Promise<any> {
    ...
    const comment = await this.prismaService.tb_product_sub_category_comment.findFirst({
      where: {
        id: id,
        deleted_at: null,
      },
   ...
   * @param productSubCategoryId - Purchase request ID / ID ของใบขอซื้อ
   * @param paginate - Pagination parameters / พารามิเตอร์การแบ่งหน้า
   * @returns Paginated list of comments with user profiles / รายการความคิดเห็นพร้อมโปรไฟล์ผู้ใช้ที่แบ่งหน้าแล้ว
   */
  async findAllByProductSubCategoryId(productSubCategoryId: string, paginate: IPaginate): Promise<any> {
    this.logger.debug(
      {
        function: "findAllByProductSubCategoryId",
    ...
    const comments = await this.prismaService.tb_product_sub_category_comment.findMany({
      where: whereQry,
      orderBy: q.orderBy().length > 0 ? q.orderBy() : { created_at: "desc" },
      ...pagination,
    });

    const total = await this.prismaService.tb_product_sub_category_comment.count({
      where: whereQry,
    });
   ...
   */
  async create(createDto: CreateProductSubCategoryComment): Promise<any> {
    this.logger.debug(
      {
        function: "create",
        createDto,
    ...
    const productSubCategory = await this.prismaService.tb_product_sub_category.findFirst({
      where: {
        id: createDto.product_sub_category_id,
      },
    });
    ...
    const comment = await this.prismaService.tb_product_sub_category_comment.create({
      data: {
        product_sub_category_id: createDto.product_sub_category_id,
   ...
   */
  async update(id: string, updateDto: UpdateProductSubCategoryComment): Promise<any> {
    this.logger.debug(
      {
        function: "update",
        id,
    ...
    const existingComment = await this.prismaService.tb_product_sub_category_comment.findFirst({
      where: {
        id: id,
        deleted_at: null,
      },
    ...
    const comment = await this.prismaService.tb_product_sub_category_comment.update({
      where: { id: id },
      data: updateData,
    });
  ...
  async delete(id: string): Promise<any> {
    ...
    // Soft delete
    await this.prismaService.tb_product_sub_category_comment.update({
      where: { id: id },
      data: {
        deleted_at: new Date(),
