...
import {
  CreateConfigRunningCodeCommentSchema,
  UpdateConfigRunningCodeCommentSchema,
  AttachmentSchema,
...
export class ConfigRunningCodeCommentController {
   ...
   */
  @MessagePattern({
    cmd: 'config-running-code-comment.find-by-id',
    service: 'config-running-code-comment',
  })
  async findById(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    await this.configRunningCodeCommentService.initializePrismaService(
      bu_code,
      user_id,
    );
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.configRunningCodeCommentService.findById(id),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'config-running-code-comment.find-all-by-config-running-code-id',
    service: 'config-running-code-comment',
  })
  async findAllByConfigRunningCodeId(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    this.logger.debug(
      { function: 'findAllByConfigRunningCodeId', payload },
      ConfigRunningCodeCommentController.name,
    );

    const { config_running_code_id, user_id, bu_code, paginate } = payload;
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.configRunningCodeCommentService.findAllByConfigRunningCodeId(
   ...
   */
  @MessagePattern({
    cmd: 'config-running-code-comment.create',
    service: 'config-running-code-comment',
  })
  async create(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const parsedData = CreateConfigRunningCodeCommentSchema.parse(data);

    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.configRunningCodeCommentService.create(parsedData),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'config-running-code-comment.update',
    service: 'config-running-code-comment',
  })
  async update(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const parsedData = UpdateConfigRunningCodeCommentSchema.parse(data);

    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.configRunningCodeCommentService.update(id, parsedData),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'config-running-code-comment.delete',
    service: 'config-running-code-comment',
  })
  async delete(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.configRunningCodeCommentService.delete(id),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'config-running-code-comment.add-attachment',
    service: 'config-running-code-comment',
  })
  async addAttachment(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.configRunningCodeCommentService.addAttachment(id, parsedAttachment),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'config-running-code-comment.remove-attachment',
    service: 'config-running-code-comment',
  })
  async removeAttachment(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.configRunningCodeCommentService.removeAttachment(id, fileToken),
    );
  }
}
