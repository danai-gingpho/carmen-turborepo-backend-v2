...
import {
  CreateConfigRunningCodeComment,
  UpdateConfigRunningCodeComment,
  Attachment,
...
export class ConfigRunningCodeCommentService {
  ...
  async findById(id: string): Promise<any> {
    ...
    const comment = await this.prismaService.tb_config_running_code_comment.findFirst({
      where: {
        id: id,
        deleted_at: null,
      },
   ...
   * @param configRunningCodeId - Purchase request ID / ID ของใบขอซื้อ
   * @param paginate - Pagination parameters / พารามิเตอร์การแบ่งหน้า
   * @returns Paginated list of comments with user profiles / รายการความคิดเห็นพร้อมโปรไฟล์ผู้ใช้ที่แบ่งหน้าแล้ว
   */
  async findAllByConfigRunningCodeId(configRunningCodeId: string, paginate: IPaginate): Promise<any> {
    this.logger.debug(
      {
        function: "findAllByConfigRunningCodeId",
        configRunningCodeId,
        user_id: this.userId,
        tenant_id: this.bu_code,
        paginate,
    ...
    const whereQry = {
      ...q.where(),
      config_running_code_id: configRunningCodeId,
      deleted_at: null,
    };

    const comments = await this.prismaService.tb_config_running_code_comment.findMany({
      where: whereQry,
      orderBy: q.orderBy().length > 0 ? q.orderBy() : { created_at: "desc" },
      ...pagination,
    });

    const total = await this.prismaService.tb_config_running_code_comment.count({
      where: whereQry,
    });
   ...
   */
  async create(createDto: CreateConfigRunningCodeComment): Promise<any> {
    this.logger.debug(
      {
        function: "create",
        createDto,
    ...
    const configRunningCode = await this.prismaService.tb_config_running_code.findFirst({
      where: {
        id: createDto.config_running_code_id,
      },
    });

    if (!configRunningCode) {
      return {
        response: {
          status: HttpStatus.NOT_FOUND,
    ...
    const comment = await this.prismaService.tb_config_running_code_comment.create({
      data: {
        config_running_code_id: createDto.config_running_code_id,
   ...
   */
  async update(id: string, updateDto: UpdateConfigRunningCodeComment): Promise<any> {
    this.logger.debug(
      {
        function: "update",
        id,
    ...
    const existingComment = await this.prismaService.tb_config_running_code_comment.findFirst({
      where: {
        id: id,
        deleted_at: null,
      },
    ...
    const comment = await this.prismaService.tb_config_running_code_comment.update({
      where: { id: id },
      data: updateData,
    });
  ...
  async delete(id: string): Promise<any> {
    ...
    // Soft delete
    await this.prismaService.tb_config_running_code_comment.update({
      where: { id: id },
      data: {
        deleted_at: new Date(),
