...
import {
  CreatePricelistDetailComment,
  UpdatePricelistDetailComment,
  Attachment,
...
export class PricelistDetailCommentService {
  ...
  async findById(id: string): Promise<any> {
    ...
    const comment = await this.prismaService.tb_pricelist_detail_comment.findFirst({
      where: {
        id: id,
        deleted_at: null,
      },
   ...
   * @param pricelistDetailId - Purchase request ID / ID ของใบขอซื้อ
   * @param paginate - Pagination parameters / พารามิเตอร์การแบ่งหน้า
   * @returns Paginated list of comments with user profiles / รายการความคิดเห็นพร้อมโปรไฟล์ผู้ใช้ที่แบ่งหน้าแล้ว
   */
  async findAllByPricelistDetailId(pricelistDetailId: string, paginate: IPaginate): Promise<any> {
    this.logger.debug(
      {
        function: "findAllByPricelistDetailId",
        pricelistDetailId,
        user_id: this.userId,
        tenant_id: this.bu_code,
        paginate,
    ...
    const whereQry = {
      ...q.where(),
      pricelist_detail_id: pricelistDetailId,
      deleted_at: null,
    };

    const comments = await this.prismaService.tb_pricelist_detail_comment.findMany({
      where: whereQry,
      orderBy: q.orderBy().length > 0 ? q.orderBy() : { created_at: "desc" },
      ...pagination,
    });

    const total = await this.prismaService.tb_pricelist_detail_comment.count({
      where: whereQry,
    });
   ...
   */
  async create(createDto: CreatePricelistDetailComment): Promise<any> {
    this.logger.debug(
      {
        function: "create",
        createDto,
    ...
    const pricelistDetail = await this.prismaService.tb_pricelist_detail.findFirst({
      where: {
        id: createDto.pricelist_detail_id,
      },
    });
    ...
    const comment = await this.prismaService.tb_pricelist_detail_comment.create({
      data: {
        pricelist_detail_id: createDto.pricelist_detail_id,
   ...
   */
  async update(id: string, updateDto: UpdatePricelistDetailComment): Promise<any> {
    this.logger.debug(
      {
        function: "update",
        id,
    ...
    const existingComment = await this.prismaService.tb_pricelist_detail_comment.findFirst({
      where: {
        id: id,
        deleted_at: null,
      },
    ...
    const comment = await this.prismaService.tb_pricelist_detail_comment.update({
      where: { id: id },
      data: updateData,
    });
  ...
  async delete(id: string): Promise<any> {
    ...
    // Soft delete
    await this.prismaService.tb_pricelist_detail_comment.update({
      where: { id: id },
      data: {
        deleted_at: new Date(),
