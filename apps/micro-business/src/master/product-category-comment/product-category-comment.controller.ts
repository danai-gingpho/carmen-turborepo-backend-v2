...
import {
  CreateProductCategoryCommentSchema,
  UpdateProductCategoryCommentSchema,
  AttachmentSchema,
...
export class ProductCategoryCommentController {
   ...
   */
  @MessagePattern({
    cmd: 'product-category-comment.find-by-id',
    service: 'product-category-comment',
  })
  async findById(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    await this.productCategoryCommentService.initializePrismaService(
      bu_code,
      user_id,
    );
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.productCategoryCommentService.findById(id),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'product-category-comment.find-all-by-product-category-id',
    service: 'product-category-comment',
  })
  async findAllByProductCategoryId(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    this.logger.debug(
      { function: 'findAllByProductCategoryId', payload },
      ProductCategoryCommentController.name,
    );

    const { product_category_id, user_id, bu_code, paginate } = payload;
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.productCategoryCommentService.findAllByProductCategoryId(
        product_category_id,
        paginate,
      ),
    );
   ...
   */
  @MessagePattern({
    cmd: 'product-category-comment.create',
    service: 'product-category-comment',
  })
  async create(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const parsedData = CreateProductCategoryCommentSchema.parse(data);

    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.productCategoryCommentService.create(parsedData),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'product-category-comment.update',
    service: 'product-category-comment',
  })
  async update(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const parsedData = UpdateProductCategoryCommentSchema.parse(data);

    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.productCategoryCommentService.update(id, parsedData),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'product-category-comment.delete',
    service: 'product-category-comment',
  })
  async delete(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.productCategoryCommentService.delete(id),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'product-category-comment.add-attachment',
    service: 'product-category-comment',
  })
  async addAttachment(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.productCategoryCommentService.addAttachment(id, parsedAttachment),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'product-category-comment.remove-attachment',
    service: 'product-category-comment',
  })
  async removeAttachment(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.productCategoryCommentService.removeAttachment(id, fileToken),
    );
  }
}
