...
import {
  CreateVendorCommentSchema,
  UpdateVendorCommentSchema,
  AttachmentSchema,
} from './dto/vendor-comment.dto';
...
export class VendorCommentController {
  private readonly logger: BackendLogger = new BackendLogger(
    VendorCommentController.name,
  );

  constructor(
    private readonly vendorCommentService: VendorCommentService,
  ) {}
   ...
   */
  @MessagePattern({
    cmd: 'vendor-comment.find-by-id',
    service: 'vendor-comment',
  })
  async findById(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    await this.vendorCommentService.initializePrismaService(
      bu_code,
      user_id,
    );
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.vendorCommentService.findById(id),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'vendor-comment.find-all-by-vendor-id',
    service: 'vendor-comment',
  })
  async findAllByVendorId(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    this.logger.debug(
      { function: 'findAllByVendorId', payload },
      VendorCommentController.name,
    );

    const { vendor_id, user_id, bu_code, paginate } = payload;
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.vendorCommentService.findAllByVendorId(
        vendor_id,
        paginate,
      ),
    );
  ...
  async create(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const parsedData = CreateVendorCommentSchema.parse(data);

    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.vendorCommentService.create(parsedData),
    );
  }
  ...
  async update(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const parsedData = UpdateVendorCommentSchema.parse(data);

    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.vendorCommentService.update(id, parsedData),
    );
  }
  ...
  async delete(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.vendorCommentService.delete(id),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'vendor-comment.add-attachment',
    service: 'vendor-comment',
  })
  async addAttachment(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.vendorCommentService.addAttachment(id, parsedAttachment),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'vendor-comment.remove-attachment',
    service: 'vendor-comment',
  })
  async removeAttachment(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.vendorCommentService.removeAttachment(id, fileToken),
    );
  }
}
