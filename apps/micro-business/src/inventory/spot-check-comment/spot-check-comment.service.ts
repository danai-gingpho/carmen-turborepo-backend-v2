...
import {
  CreateSpotCheckComment,
  UpdateSpotCheckComment,
  Attachment,
...
export class SpotCheckCommentService {
  ...
  private readonly logger: BackendLogger = new BackendLogger(SpotCheckCommentService.name);
  ...
  async findById(id: string): Promise<any> {
    ...
    const comment = await this.prismaService.tb_spot_check_comment.findFirst({
      where: {
        id: id,
        deleted_at: null,
      },
   ...
   * @param spotCheckId - Purchase request ID / ID ของใบขอซื้อ
   * @param paginate - Pagination parameters / พารามิเตอร์การแบ่งหน้า
   * @returns Paginated list of comments with user profiles / รายการความคิดเห็นพร้อมโปรไฟล์ผู้ใช้ที่แบ่งหน้าแล้ว
   */
  async findAllBySpotCheckId(spotCheckId: string, paginate: IPaginate): Promise<any> {
    this.logger.debug(
      {
        function: "findAllBySpotCheckId",
        spotCheckId,
        user_id: this.userId,
        tenant_id: this.bu_code,
        paginate,
    ...
    const comments = await this.prismaService.tb_spot_check_comment.findMany({
      where: whereQry,
      orderBy: q.orderBy().length > 0 ? q.orderBy() : { created_at: "desc" },
      ...pagination,
    });

    const total = await this.prismaService.tb_spot_check_comment.count({
      where: whereQry,
    });
   ...
   */
  async create(createDto: CreateSpotCheckComment): Promise<any> {
    this.logger.debug(
      {
        function: "create",
        createDto,
    ...
    const spotCheck = await this.prismaService.tb_spot_check.findFirst({
      where: {
        id: createDto.spot_check_id,
      },
    });
    ...
    const comment = await this.prismaService.tb_spot_check_comment.create({
      data: {
        spot_check_id: createDto.spot_check_id,
   ...
   */
  async update(id: string, updateDto: UpdateSpotCheckComment): Promise<any> {
    this.logger.debug(
      {
        function: "update",
        id,
    ...
    const existingComment = await this.prismaService.tb_spot_check_comment.findFirst({
      where: {
        id: id,
        deleted_at: null,
      },
    ...
    const comment = await this.prismaService.tb_spot_check_comment.update({
      where: { id: id },
      data: updateData,
    });
  ...
  async delete(id: string): Promise<any> {
    ...
    // Soft delete
    await this.prismaService.tb_spot_check_comment.update({
      where: { id: id },
      data: {
        deleted_at: new Date(),
