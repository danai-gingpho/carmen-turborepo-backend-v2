...
import {
  CreateSpotCheckCommentSchema,
  UpdateSpotCheckCommentSchema,
  AttachmentSchema,
...
export class SpotCheckCommentController {
   ...
   */
  @MessagePattern({
    cmd: 'spot-check-comment.find-by-id',
    service: 'spot-check-comment',
  })
  async findById(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    await this.spotCheckCommentService.initializePrismaService(
      bu_code,
      user_id,
    );
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.spotCheckCommentService.findById(id),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'spot-check-comment.find-all-by-spot-check-id',
    service: 'spot-check-comment',
  })
  async findAllBySpotCheckId(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    this.logger.debug(
      { function: 'findAllBySpotCheckId', payload },
      SpotCheckCommentController.name,
    );

    const { spot_check_id, user_id, bu_code, paginate } = payload;
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.spotCheckCommentService.findAllBySpotCheckId(
        spot_check_id,
        paginate,
      ),
    );
  ...
  async create(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const parsedData = CreateSpotCheckCommentSchema.parse(data);

    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.spotCheckCommentService.create(parsedData),
    );
  }
  ...
  async update(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const parsedData = UpdateSpotCheckCommentSchema.parse(data);

    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.spotCheckCommentService.update(id, parsedData),
    );
  }
  ...
  async delete(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.spotCheckCommentService.delete(id),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'spot-check-comment.add-attachment',
    service: 'spot-check-comment',
  })
  async addAttachment(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.spotCheckCommentService.addAttachment(id, parsedAttachment),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'spot-check-comment.remove-attachment',
    service: 'spot-check-comment',
  })
  async removeAttachment(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.spotCheckCommentService.removeAttachment(id, fileToken),
    );
  }
}
