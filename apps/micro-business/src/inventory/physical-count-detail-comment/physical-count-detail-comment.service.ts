...
import {
  CreatePhysicalCountDetailComment,
  UpdatePhysicalCountDetailComment,
  Attachment,
...
export class PhysicalCountDetailCommentService {
  ...
  private readonly logger: BackendLogger = new BackendLogger(PhysicalCountDetailCommentService.name);
  ...
  async findById(id: string): Promise<any> {
    ...
    const comment = await this.prismaService.tb_physical_count_detail_comment.findFirst({
      where: {
        id: id,
        deleted_at: null,
      },
   ...
   * @param physicalCountDetailId - Purchase request ID / ID ของใบขอซื้อ
   * @param paginate - Pagination parameters / พารามิเตอร์การแบ่งหน้า
   * @returns Paginated list of comments with user profiles / รายการความคิดเห็นพร้อมโปรไฟล์ผู้ใช้ที่แบ่งหน้าแล้ว
   */
  async findAllByPhysicalCountDetailId(physicalCountDetailId: string, paginate: IPaginate): Promise<any> {
    this.logger.debug(
      {
        function: "findAllByPhysicalCountDetailId",
    ...
    const whereQry = {
      ...q.where(),
      physical_count_detail_id: physicalCountDetailId,
      deleted_at: null,
    };

    const comments = await this.prismaService.tb_physical_count_detail_comment.findMany({
      where: whereQry,
      orderBy: q.orderBy().length > 0 ? q.orderBy() : { created_at: "desc" },
      ...pagination,
    });

    const total = await this.prismaService.tb_physical_count_detail_comment.count({
      where: whereQry,
    });
   ...
   */
  async create(createDto: CreatePhysicalCountDetailComment): Promise<any> {
    this.logger.debug(
      {
        function: "create",
        createDto,
    ...
    const physicalCountDetail = await this.prismaService.tb_physical_count_detail.findFirst({
      where: {
        id: createDto.physical_count_detail_id,
      },
    });
    ...
    const comment = await this.prismaService.tb_physical_count_detail_comment.create({
      data: {
        physical_count_detail_id: createDto.physical_count_detail_id,
   ...
   */
  async update(id: string, updateDto: UpdatePhysicalCountDetailComment): Promise<any> {
    this.logger.debug(
      {
        function: "update",
        id,
    ...
    const existingComment = await this.prismaService.tb_physical_count_detail_comment.findFirst({
      where: {
        id: id,
        deleted_at: null,
      },
    ...
    const comment = await this.prismaService.tb_physical_count_detail_comment.update({
      where: { id: id },
      data: updateData,
    });
  ...
  async delete(id: string): Promise<any> {
    ...
    // Soft delete
    await this.prismaService.tb_physical_count_detail_comment.update({
      where: { id: id },
      data: {
        deleted_at: new Date(),
