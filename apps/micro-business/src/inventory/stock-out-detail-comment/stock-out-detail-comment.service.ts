...
import {
  CreateStockOutDetailComment,
  UpdateStockOutDetailComment,
  Attachment,
...
export class StockOutDetailCommentService {
  ...
  private readonly logger: BackendLogger = new BackendLogger(StockOutDetailCommentService.name);
  ...
  async findById(id: string): Promise<any> {
    ...
    const comment = await this.prismaService.tb_stock_out_detail_comment.findFirst({
      where: {
        id: id,
        deleted_at: null,
      },
   ...
   * @param stockOutDetailId - Purchase request ID / ID ของใบขอซื้อ
   * @param paginate - Pagination parameters / พารามิเตอร์การแบ่งหน้า
   * @returns Paginated list of comments with user profiles / รายการความคิดเห็นพร้อมโปรไฟล์ผู้ใช้ที่แบ่งหน้าแล้ว
   */
  async findAllByStockOutDetailId(stockOutDetailId: string, paginate: IPaginate): Promise<any> {
    this.logger.debug(
      {
        function: "findAllByStockOutDetailId",
        stockOutDetailId,
        user_id: this.userId,
        tenant_id: this.bu_code,
        paginate,
    ...
    const whereQry = {
      ...q.where(),
      stock_out_detail_id: stockOutDetailId,
      deleted_at: null,
    };

    const comments = await this.prismaService.tb_stock_out_detail_comment.findMany({
      where: whereQry,
      orderBy: q.orderBy().length > 0 ? q.orderBy() : { created_at: "desc" },
      ...pagination,
    });

    const total = await this.prismaService.tb_stock_out_detail_comment.count({
      where: whereQry,
    });
   ...
   */
  async create(createDto: CreateStockOutDetailComment): Promise<any> {
    this.logger.debug(
      {
        function: "create",
        createDto,
    ...
    const stockOutDetail = await this.prismaService.tb_stock_out_detail.findFirst({
      where: {
        id: createDto.stock_out_detail_id,
      },
    });
    ...
    const comment = await this.prismaService.tb_stock_out_detail_comment.create({
      data: {
        stock_out_detail_id: createDto.stock_out_detail_id,
   ...
   */
  async update(id: string, updateDto: UpdateStockOutDetailComment): Promise<any> {
    this.logger.debug(
      {
        function: "update",
        id,
    ...
    const existingComment = await this.prismaService.tb_stock_out_detail_comment.findFirst({
      where: {
        id: id,
        deleted_at: null,
      },
    ...
    const comment = await this.prismaService.tb_stock_out_detail_comment.update({
      where: { id: id },
      data: updateData,
    });
  ...
  async delete(id: string): Promise<any> {
    ...
    // Soft delete
    await this.prismaService.tb_stock_out_detail_comment.update({
      where: { id: id },
      data: {
        deleted_at: new Date(),
