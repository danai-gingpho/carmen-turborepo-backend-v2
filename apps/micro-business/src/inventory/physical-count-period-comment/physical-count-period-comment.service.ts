...
import {
  CreatePhysicalCountPeriodComment,
  UpdatePhysicalCountPeriodComment,
  Attachment,
...
export class PhysicalCountPeriodCommentService {
  ...
  private readonly logger: BackendLogger = new BackendLogger(PhysicalCountPeriodCommentService.name);
  ...
  async findById(id: string): Promise<any> {
    ...
    const comment = await this.prismaService.tb_physical_count_period_comment.findFirst({
      where: {
        id: id,
        deleted_at: null,
      },
   ...
   * @param physicalCountPeriodId - Purchase request ID / ID ของใบขอซื้อ
   * @param paginate - Pagination parameters / พารามิเตอร์การแบ่งหน้า
   * @returns Paginated list of comments with user profiles / รายการความคิดเห็นพร้อมโปรไฟล์ผู้ใช้ที่แบ่งหน้าแล้ว
   */
  async findAllByPhysicalCountPeriodId(physicalCountPeriodId: string, paginate: IPaginate): Promise<any> {
    this.logger.debug(
      {
        function: "findAllByPhysicalCountPeriodId",
        physicalCountPeriodId,
        user_id: this.userId,
        tenant_id: this.bu_code,
        paginate,
      },
    ...
    const comments = await this.prismaService.tb_physical_count_period_comment.findMany({
      where: whereQry,
      orderBy: q.orderBy().length > 0 ? q.orderBy() : { created_at: "desc" },
      ...pagination,
    });

    const total = await this.prismaService.tb_physical_count_period_comment.count({
      where: whereQry,
    });
   ...
   */
  async create(createDto: CreatePhysicalCountPeriodComment): Promise<any> {
    this.logger.debug(
      {
        function: "create",
        createDto,
    ...
    const physicalCountPeriod = await this.prismaService.tb_physical_count_period.findFirst({
      where: {
        id: createDto.physical_count_period_id,
      },
    });

    if (!physicalCountPeriod) {
      return {
        response: {
          status: HttpStatus.NOT_FOUND,
    ...
    const comment = await this.prismaService.tb_physical_count_period_comment.create({
      data: {
        physical_count_period_id: createDto.physical_count_period_id,
   ...
   */
  async update(id: string, updateDto: UpdatePhysicalCountPeriodComment): Promise<any> {
    this.logger.debug(
      {
        function: "update",
        id,
    ...
    const existingComment = await this.prismaService.tb_physical_count_period_comment.findFirst({
      where: {
        id: id,
        deleted_at: null,
      },
    ...
    const comment = await this.prismaService.tb_physical_count_period_comment.update({
      where: { id: id },
      data: updateData,
    });
  ...
  async delete(id: string): Promise<any> {
    ...
    // Soft delete
    await this.prismaService.tb_physical_count_period_comment.update({
      where: { id: id },
      data: {
        deleted_at: new Date(),
