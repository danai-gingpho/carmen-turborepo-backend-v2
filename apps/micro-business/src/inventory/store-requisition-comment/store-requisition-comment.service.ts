...
import {
  CreateStoreRequisitionComment,
  UpdateStoreRequisitionComment,
  Attachment,
...
export class StoreRequisitionCommentService {
  ...
  private readonly logger: BackendLogger = new BackendLogger(StoreRequisitionCommentService.name);
  ...
  async findById(id: string): Promise<any> {
    ...
    const comment = await this.prismaService.tb_store_requisition_comment.findFirst({
      where: {
        id: id,
        deleted_at: null,
      },
   ...
   * @param storeRequisitionId - Purchase request ID / ID ของใบขอซื้อ
   * @param paginate - Pagination parameters / พารามิเตอร์การแบ่งหน้า
   * @returns Paginated list of comments with user profiles / รายการความคิดเห็นพร้อมโปรไฟล์ผู้ใช้ที่แบ่งหน้าแล้ว
   */
  async findAllByStoreRequisitionId(storeRequisitionId: string, paginate: IPaginate): Promise<any> {
    this.logger.debug(
      {
        function: "findAllByStoreRequisitionId",
        storeRequisitionId,
        user_id: this.userId,
        tenant_id: this.bu_code,
        paginate,
    ...
    const comments = await this.prismaService.tb_store_requisition_comment.findMany({
      where: whereQry,
      orderBy: q.orderBy().length > 0 ? q.orderBy() : { created_at: "desc" },
      ...pagination,
    });

    const total = await this.prismaService.tb_store_requisition_comment.count({
      where: whereQry,
    });
   ...
   */
  async create(createDto: CreateStoreRequisitionComment): Promise<any> {
    this.logger.debug(
      {
        function: "create",
        createDto,
    ...
    const storeRequisition = await this.prismaService.tb_store_requisition.findFirst({
      where: {
        id: createDto.store_requisition_id,
      },
    });
    ...
    const comment = await this.prismaService.tb_store_requisition_comment.create({
      data: {
        store_requisition_id: createDto.store_requisition_id,
   ...
   */
  async update(id: string, updateDto: UpdateStoreRequisitionComment): Promise<any> {
    this.logger.debug(
      {
        function: "update",
        id,
    ...
    const existingComment = await this.prismaService.tb_store_requisition_comment.findFirst({
      where: {
        id: id,
        deleted_at: null,
      },
    ...
    const comment = await this.prismaService.tb_store_requisition_comment.update({
      where: { id: id },
      data: updateData,
    });
  ...
  async delete(id: string): Promise<any> {
    ...
    // Soft delete
    await this.prismaService.tb_store_requisition_comment.update({
      where: { id: id },
      data: {
        deleted_at: new Date(),
