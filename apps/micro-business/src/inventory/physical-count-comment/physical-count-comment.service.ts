...
import {
  CreatePhysicalCountComment,
  UpdatePhysicalCountComment,
  Attachment,
...
export class PhysicalCountCommentService {
  ...
  private readonly logger: BackendLogger = new BackendLogger(PhysicalCountCommentService.name);
  ...
  async findById(id: string): Promise<any> {
    ...
    const comment = await this.prismaService.tb_physical_count_comment.findFirst({
      where: {
        id: id,
        deleted_at: null,
      },
   ...
   * @param physicalCountId - Purchase request ID / ID ของใบขอซื้อ
   * @param paginate - Pagination parameters / พารามิเตอร์การแบ่งหน้า
   * @returns Paginated list of comments with user profiles / รายการความคิดเห็นพร้อมโปรไฟล์ผู้ใช้ที่แบ่งหน้าแล้ว
   */
  async findAllByPhysicalCountId(physicalCountId: string, paginate: IPaginate): Promise<any> {
    this.logger.debug(
      {
        function: "findAllByPhysicalCountId",
        physicalCountId,
        user_id: this.userId,
        tenant_id: this.bu_code,
        paginate,
    ...
    const comments = await this.prismaService.tb_physical_count_comment.findMany({
      where: whereQry,
      orderBy: q.orderBy().length > 0 ? q.orderBy() : { created_at: "desc" },
      ...pagination,
    });

    const total = await this.prismaService.tb_physical_count_comment.count({
      where: whereQry,
    });
   ...
   */
  async create(createDto: CreatePhysicalCountComment): Promise<any> {
    this.logger.debug(
      {
        function: "create",
        createDto,
    ...
    const physicalCount = await this.prismaService.tb_physical_count.findFirst({
      where: {
        id: createDto.physical_count_id,
      },
    });
    ...
    const comment = await this.prismaService.tb_physical_count_comment.create({
      data: {
        physical_count_id: createDto.physical_count_id,
   ...
   */
  async update(id: string, updateDto: UpdatePhysicalCountComment): Promise<any> {
    this.logger.debug(
      {
        function: "update",
        id,
    ...
    const existingComment = await this.prismaService.tb_physical_count_comment.findFirst({
      where: {
        id: id,
        deleted_at: null,
      },
    ...
    const comment = await this.prismaService.tb_physical_count_comment.update({
      where: { id: id },
      data: updateData,
    });
  ...
  async delete(id: string): Promise<any> {
    ...
    // Soft delete
    await this.prismaService.tb_physical_count_comment.update({
      where: { id: id },
      data: {
        deleted_at: new Date(),
