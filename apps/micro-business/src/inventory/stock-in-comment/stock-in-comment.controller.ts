...
import {
  CreateStockInCommentSchema,
  UpdateStockInCommentSchema,
  AttachmentSchema,
...
export class StockInCommentController {
   ...
   */
  @MessagePattern({
    cmd: 'stock-in-comment.find-by-id',
    service: 'stock-in-comment',
  })
  async findById(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    await this.stockInCommentService.initializePrismaService(
      bu_code,
      user_id,
    );
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.stockInCommentService.findById(id),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'stock-in-comment.find-all-by-stock-in-id',
    service: 'stock-in-comment',
  })
  async findAllByStockInId(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    this.logger.debug(
      { function: 'findAllByStockInId', payload },
      StockInCommentController.name,
    );

    const { stock_in_id, user_id, bu_code, paginate } = payload;
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.stockInCommentService.findAllByStockInId(
        stock_in_id,
        paginate,
      ),
    );
  ...
  async create(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const parsedData = CreateStockInCommentSchema.parse(data);

    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.stockInCommentService.create(parsedData),
    );
  }
  ...
  async update(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const parsedData = UpdateStockInCommentSchema.parse(data);

    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.stockInCommentService.update(id, parsedData),
    );
  }
  ...
  async delete(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.stockInCommentService.delete(id),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'stock-in-comment.add-attachment',
    service: 'stock-in-comment',
  })
  async addAttachment(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.stockInCommentService.addAttachment(id, parsedAttachment),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'stock-in-comment.remove-attachment',
    service: 'stock-in-comment',
  })
  async removeAttachment(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.stockInCommentService.removeAttachment(id, fileToken),
    );
  }
}
