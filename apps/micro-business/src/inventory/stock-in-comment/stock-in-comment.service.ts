...
import {
  CreateStockInComment,
  UpdateStockInComment,
  Attachment,
...
export class StockInCommentService {
  ...
  private readonly logger: BackendLogger = new BackendLogger(StockInCommentService.name);
  ...
  async findById(id: string): Promise<any> {
    ...
    const comment = await this.prismaService.tb_stock_in_comment.findFirst({
      where: {
        id: id,
        deleted_at: null,
      },
   ...
   * @param stockInId - Purchase request ID / ID ของใบขอซื้อ
   * @param paginate - Pagination parameters / พารามิเตอร์การแบ่งหน้า
   * @returns Paginated list of comments with user profiles / รายการความคิดเห็นพร้อมโปรไฟล์ผู้ใช้ที่แบ่งหน้าแล้ว
   */
  async findAllByStockInId(stockInId: string, paginate: IPaginate): Promise<any> {
    this.logger.debug(
      {
        function: "findAllByStockInId",
        stockInId,
        user_id: this.userId,
        tenant_id: this.bu_code,
        paginate,
    ...
    const comments = await this.prismaService.tb_stock_in_comment.findMany({
      where: whereQry,
      orderBy: q.orderBy().length > 0 ? q.orderBy() : { created_at: "desc" },
      ...pagination,
    });

    const total = await this.prismaService.tb_stock_in_comment.count({
      where: whereQry,
    });
   ...
   */
  async create(createDto: CreateStockInComment): Promise<any> {
    this.logger.debug(
      {
        function: "create",
        createDto,
    ...
    const stockIn = await this.prismaService.tb_stock_in.findFirst({
      where: {
        id: createDto.stock_in_id,
      },
    });
    ...
    const comment = await this.prismaService.tb_stock_in_comment.create({
      data: {
        stock_in_id: createDto.stock_in_id,
   ...
   */
  async update(id: string, updateDto: UpdateStockInComment): Promise<any> {
    this.logger.debug(
      {
        function: "update",
        id,
    ...
    const existingComment = await this.prismaService.tb_stock_in_comment.findFirst({
      where: {
        id: id,
        deleted_at: null,
      },
    ...
    const comment = await this.prismaService.tb_stock_in_comment.update({
      where: { id: id },
      data: updateData,
    });
  ...
  async delete(id: string): Promise<any> {
    ...
    // Soft delete
    await this.prismaService.tb_stock_in_comment.update({
      where: { id: id },
      data: {
        deleted_at: new Date(),
