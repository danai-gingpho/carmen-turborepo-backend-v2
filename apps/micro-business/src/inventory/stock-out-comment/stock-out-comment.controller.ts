...
import {
  CreateStockOutCommentSchema,
  UpdateStockOutCommentSchema,
  AttachmentSchema,
...
export class StockOutCommentController {
   ...
   */
  @MessagePattern({
    cmd: 'stock-out-comment.find-by-id',
    service: 'stock-out-comment',
  })
  async findById(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    await this.stockOutCommentService.initializePrismaService(
      bu_code,
      user_id,
    );
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.stockOutCommentService.findById(id),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'stock-out-comment.find-all-by-stock-out-id',
    service: 'stock-out-comment',
  })
  async findAllByStockOutId(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    this.logger.debug(
      { function: 'findAllByStockOutId', payload },
      StockOutCommentController.name,
    );

    const { stock_out_id, user_id, bu_code, paginate } = payload;
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.stockOutCommentService.findAllByStockOutId(
        stock_out_id,
        paginate,
      ),
    );
  ...
  async create(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const parsedData = CreateStockOutCommentSchema.parse(data);

    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.stockOutCommentService.create(parsedData),
    );
  }
  ...
  async update(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const parsedData = UpdateStockOutCommentSchema.parse(data);

    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.stockOutCommentService.update(id, parsedData),
    );
  }
  ...
  async delete(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.stockOutCommentService.delete(id),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'stock-out-comment.add-attachment',
    service: 'stock-out-comment',
  })
  async addAttachment(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.stockOutCommentService.addAttachment(id, parsedAttachment),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'stock-out-comment.remove-attachment',
    service: 'stock-out-comment',
  })
  async removeAttachment(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.stockOutCommentService.removeAttachment(id, fileToken),
    );
  }
}
