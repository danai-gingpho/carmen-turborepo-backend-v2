...
import {
  CreatePurchaseOrderDetailComment,
  UpdatePurchaseOrderDetailComment,
  Attachment,
...
export class PurchaseOrderDetailCommentService {
  ...
  private readonly logger: BackendLogger = new BackendLogger(PurchaseOrderDetailCommentService.name);
  ...
  async findById(id: string): Promise<any> {
    ...
    const comment = await this.prismaService.tb_purchase_order_detail_comment.findFirst({
      where: {
        id: id,
        deleted_at: null,
      },
   ...
   * @param purchaseOrderDetailId - Purchase request ID / ID ของใบขอซื้อ
   * @param paginate - Pagination parameters / พารามิเตอร์การแบ่งหน้า
   * @returns Paginated list of comments with user profiles / รายการความคิดเห็นพร้อมโปรไฟล์ผู้ใช้ที่แบ่งหน้าแล้ว
   */
  async findAllByPurchaseOrderDetailId(purchaseOrderDetailId: string, paginate: IPaginate): Promise<any> {
    this.logger.debug(
      {
        function: "findAllByPurchaseOrderDetailId",
    ...
    const comments = await this.prismaService.tb_purchase_order_detail_comment.findMany({
      where: whereQry,
      orderBy: q.orderBy().length > 0 ? q.orderBy() : { created_at: "desc" },
      ...pagination,
    });

    const total = await this.prismaService.tb_purchase_order_detail_comment.count({
      where: whereQry,
    });
   ...
   */
  async create(createDto: CreatePurchaseOrderDetailComment): Promise<any> {
    this.logger.debug(
      {
        function: "create",
        createDto,
    ...
    const purchaseOrderDetail = await this.prismaService.tb_purchase_order_detail.findFirst({
      where: {
        id: createDto.purchase_order_detail_id,
      },
    });
    ...
    const comment = await this.prismaService.tb_purchase_order_detail_comment.create({
      data: {
        purchase_order_detail_id: createDto.purchase_order_detail_id,
   ...
   */
  async update(id: string, updateDto: UpdatePurchaseOrderDetailComment): Promise<any> {
    this.logger.debug(
      {
        function: "update",
        id,
    ...
    const existingComment = await this.prismaService.tb_purchase_order_detail_comment.findFirst({
      where: {
        id: id,
        deleted_at: null,
      },
    ...
    const comment = await this.prismaService.tb_purchase_order_detail_comment.update({
      where: { id: id },
      data: updateData,
    });
  ...
  async delete(id: string): Promise<any> {
    ...
    // Soft delete
    await this.prismaService.tb_purchase_order_detail_comment.update({
      where: { id: id },
      data: {
        deleted_at: new Date(),
