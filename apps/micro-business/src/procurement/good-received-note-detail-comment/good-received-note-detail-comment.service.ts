...
import {
  CreateGoodReceivedNoteDetailComment,
  UpdateGoodReceivedNoteDetailComment,
  Attachment,
...
export class GoodReceivedNoteDetailCommentService {
  ...
  private readonly logger: BackendLogger = new BackendLogger(GoodReceivedNoteDetailCommentService.name);
  ...
  async findById(id: string): Promise<any> {
    ...
    const comment = await this.prismaService.tb_good_received_note_detail_comment.findFirst({
      where: {
        id: id,
        deleted_at: null,
      },
   ...
   * @param goodReceivedNoteDetailId - Purchase request ID / ID ของใบขอซื้อ
   * @param paginate - Pagination parameters / พารามิเตอร์การแบ่งหน้า
   * @returns Paginated list of comments with user profiles / รายการความคิดเห็นพร้อมโปรไฟล์ผู้ใช้ที่แบ่งหน้าแล้ว
   */
  async findAllByGoodReceivedNoteDetailId(goodReceivedNoteDetailId: string, paginate: IPaginate): Promise<any> {
    this.logger.debug(
      {
        function: "findAllByGoodReceivedNoteDetailId",
        goodReceivedNoteDetailId,
        user_id: this.userId,
        tenant_id: this.bu_code,
        paginate,
      },
    ...
    const whereQry = {
      ...q.where(),
      good_received_note_detail_id: goodReceivedNoteDetailId,
      deleted_at: null,
    };

    const comments = await this.prismaService.tb_good_received_note_detail_comment.findMany({
      where: whereQry,
      orderBy: q.orderBy().length > 0 ? q.orderBy() : { created_at: "desc" },
      ...pagination,
    });

    const total = await this.prismaService.tb_good_received_note_detail_comment.count({
      where: whereQry,
    });
   ...
   */
  async create(createDto: CreateGoodReceivedNoteDetailComment): Promise<any> {
    this.logger.debug(
      {
        function: "create",
        createDto,
    ...
    const goodReceivedNoteDetail = await this.prismaService.tb_good_received_note_detail.findFirst({
      where: {
        id: createDto.good_received_note_detail_id,
      },
    });

    if (!goodReceivedNoteDetail) {
      return {
        response: {
          status: HttpStatus.NOT_FOUND,
    ...
    const comment = await this.prismaService.tb_good_received_note_detail_comment.create({
      data: {
        good_received_note_detail_id: createDto.good_received_note_detail_id,
   ...
   */
  async update(id: string, updateDto: UpdateGoodReceivedNoteDetailComment): Promise<any> {
    this.logger.debug(
      {
        function: "update",
        id,
    ...
    const existingComment = await this.prismaService.tb_good_received_note_detail_comment.findFirst({
      where: {
        id: id,
        deleted_at: null,
      },
    ...
    const comment = await this.prismaService.tb_good_received_note_detail_comment.update({
      where: { id: id },
      data: updateData,
    });
  ...
  async delete(id: string): Promise<any> {
    ...
    // Soft delete
    await this.prismaService.tb_good_received_note_detail_comment.update({
      where: { id: id },
      data: {
        deleted_at: new Date(),
