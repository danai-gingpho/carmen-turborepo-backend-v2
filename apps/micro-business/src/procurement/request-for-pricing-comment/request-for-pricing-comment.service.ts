...
import {
  CreateRequestForPricingComment,
  UpdateRequestForPricingComment,
  Attachment,
...
export class RequestForPricingCommentService {
  ...
  private readonly logger: BackendLogger = new BackendLogger(RequestForPricingCommentService.name);
  ...
  async findById(id: string): Promise<any> {
    ...
    const comment = await this.prismaService.tb_request_for_pricing_comment.findFirst({
      where: {
        id: id,
        deleted_at: null,
      },
   ...
   * @param requestForPricingId - Purchase request ID / ID ของใบขอซื้อ
   * @param paginate - Pagination parameters / พารามิเตอร์การแบ่งหน้า
   * @returns Paginated list of comments with user profiles / รายการความคิดเห็นพร้อมโปรไฟล์ผู้ใช้ที่แบ่งหน้าแล้ว
   */
  async findAllByRequestForPricingId(requestForPricingId: string, paginate: IPaginate): Promise<any> {
    this.logger.debug(
      {
        function: "findAllByRequestForPricingId",
        requestForPricingId,
        user_id: this.userId,
        tenant_id: this.bu_code,
        paginate,
    ...
    const whereQry = {
      ...q.where(),
      request_for_pricing_id: requestForPricingId,
      deleted_at: null,
    };

    const comments = await this.prismaService.tb_request_for_pricing_comment.findMany({
      where: whereQry,
      orderBy: q.orderBy().length > 0 ? q.orderBy() : { created_at: "desc" },
      ...pagination,
    });

    const total = await this.prismaService.tb_request_for_pricing_comment.count({
      where: whereQry,
    });
   ...
   */
  async create(createDto: CreateRequestForPricingComment): Promise<any> {
    this.logger.debug(
      {
        function: "create",
        createDto,
    ...
    const requestForPricing = await this.prismaService.tb_request_for_pricing.findFirst({
      where: {
        id: createDto.request_for_pricing_id,
      },
    });
    ...
    const comment = await this.prismaService.tb_request_for_pricing_comment.create({
      data: {
        request_for_pricing_id: createDto.request_for_pricing_id,
   ...
   */
  async update(id: string, updateDto: UpdateRequestForPricingComment): Promise<any> {
    this.logger.debug(
      {
        function: "update",
        id,
    ...
    const existingComment = await this.prismaService.tb_request_for_pricing_comment.findFirst({
      where: {
        id: id,
        deleted_at: null,
      },
    ...
    const comment = await this.prismaService.tb_request_for_pricing_comment.update({
      where: { id: id },
      data: updateData,
    });
  ...
  async delete(id: string): Promise<any> {
    ...
    // Soft delete
    await this.prismaService.tb_request_for_pricing_comment.update({
      where: { id: id },
      data: {
        deleted_at: new Date(),
