...
import {
  CreateCreditNoteCommentSchema,
  UpdateCreditNoteCommentSchema,
  AttachmentSchema,
...
export class CreditNoteCommentController {
   ...
   */
  @MessagePattern({
    cmd: 'credit-note-comment.find-by-id',
    service: 'credit-note-comment',
  })
  async findById(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    await this.creditNoteCommentService.initializePrismaService(
      bu_code,
      user_id,
    );
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.creditNoteCommentService.findById(id),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'credit-note-comment.find-all-by-credit-note-id',
    service: 'credit-note-comment',
  })
  async findAllByCreditNoteId(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    this.logger.debug(
      { function: 'findAllByCreditNoteId', payload },
      CreditNoteCommentController.name,
    );

    const { credit_note_id, user_id, bu_code, paginate } = payload;
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.creditNoteCommentService.findAllByCreditNoteId(
        credit_note_id,
        paginate,
      ),
    );
   ...
   */
  @MessagePattern({
    cmd: 'credit-note-comment.create',
    service: 'credit-note-comment',
  })
  async create(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const parsedData = CreateCreditNoteCommentSchema.parse(data);

    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.creditNoteCommentService.create(parsedData),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'credit-note-comment.update',
    service: 'credit-note-comment',
  })
  async update(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const parsedData = UpdateCreditNoteCommentSchema.parse(data);

    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.creditNoteCommentService.update(id, parsedData),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'credit-note-comment.delete',
    service: 'credit-note-comment',
  })
  async delete(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.creditNoteCommentService.delete(id),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'credit-note-comment.add-attachment',
    service: 'credit-note-comment',
  })
  async addAttachment(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.creditNoteCommentService.addAttachment(id, parsedAttachment),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'credit-note-comment.remove-attachment',
    service: 'credit-note-comment',
  })
  async removeAttachment(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.creditNoteCommentService.removeAttachment(id, fileToken),
    );
  }
}
