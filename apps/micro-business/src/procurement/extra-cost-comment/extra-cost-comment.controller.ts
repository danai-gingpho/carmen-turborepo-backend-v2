...
import {
  CreateExtraCostCommentSchema,
  UpdateExtraCostCommentSchema,
  AttachmentSchema,
...
export class ExtraCostCommentController {
   ...
   */
  @MessagePattern({
    cmd: 'extra-cost-comment.find-by-id',
    service: 'extra-cost-comment',
  })
  async findById(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    await this.extraCostCommentService.initializePrismaService(
      bu_code,
      user_id,
    );
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.extraCostCommentService.findById(id),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'extra-cost-comment.find-all-by-extra-cost-id',
    service: 'extra-cost-comment',
  })
  async findAllByExtraCostId(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    this.logger.debug(
      { function: 'findAllByExtraCostId', payload },
      ExtraCostCommentController.name,
    );

    const { extra_cost_id, user_id, bu_code, paginate } = payload;
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.extraCostCommentService.findAllByExtraCostId(
        extra_cost_id,
        paginate,
      ),
    );
  ...
  async create(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const parsedData = CreateExtraCostCommentSchema.parse(data);

    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.extraCostCommentService.create(parsedData),
    );
  }
  ...
  async update(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const parsedData = UpdateExtraCostCommentSchema.parse(data);

    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.extraCostCommentService.update(id, parsedData),
    );
  }
  ...
  async delete(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.extraCostCommentService.delete(id),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'extra-cost-comment.add-attachment',
    service: 'extra-cost-comment',
  })
  async addAttachment(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.extraCostCommentService.addAttachment(id, parsedAttachment),
    );
  }
   ...
   */
  @MessagePattern({
    cmd: 'extra-cost-comment.remove-attachment',
    service: 'extra-cost-comment',
  })
  async removeAttachment(@Body() payload: MicroservicePayload): Promise<MicroserviceResponse> {
    ...
    const auditContext = this.createAuditContext(payload);
    return runWithAuditContext(auditContext, () =>
      this.extraCostCommentService.removeAttachment(id, fileToken),
    );
  }
}
