...
import {
  CreatePurchaseRequestDetailComment,
  UpdatePurchaseRequestDetailComment,
  Attachment,
...
export class PurchaseRequestDetailCommentService {
  ...
  private readonly logger: BackendLogger = new BackendLogger(PurchaseRequestDetailCommentService.name);
  ...
  async findById(id: string): Promise<any> {
    ...
    const comment = await this.prismaService.tb_purchase_request_detail_comment.findFirst({
      where: {
        id: id,
        deleted_at: null,
      },
   ...
   * @param purchaseRequestDetailId - Purchase request ID / ID ของใบขอซื้อ
   * @param paginate - Pagination parameters / พารามิเตอร์การแบ่งหน้า
   * @returns Paginated list of comments with user profiles / รายการความคิดเห็นพร้อมโปรไฟล์ผู้ใช้ที่แบ่งหน้าแล้ว
   */
  async findAllByPurchaseRequestDetailId(purchaseRequestDetailId: string, paginate: IPaginate): Promise<any> {
    this.logger.debug(
      {
        function: "findAllByPurchaseRequestDetailId",
    ...
    const whereQry = {
      ...q.where(),
      purchase_request_detail_id: purchaseRequestDetailId,
      deleted_at: null,
    };

    const comments = await this.prismaService.tb_purchase_request_detail_comment.findMany({
      where: whereQry,
      orderBy: q.orderBy().length > 0 ? q.orderBy() : { created_at: "desc" },
      ...pagination,
    });

    const total = await this.prismaService.tb_purchase_request_detail_comment.count({
      where: whereQry,
    });
   ...
   */
  async create(createDto: CreatePurchaseRequestDetailComment): Promise<any> {
    this.logger.debug(
      {
        function: "create",
        createDto,
    ...
    const purchaseRequestDetail = await this.prismaService.tb_purchase_request_detail.findFirst({
      where: {
        id: createDto.purchase_request_detail_id,
      },
    });
    ...
    const comment = await this.prismaService.tb_purchase_request_detail_comment.create({
      data: {
        purchase_request_detail_id: createDto.purchase_request_detail_id,
   ...
   */
  async update(id: string, updateDto: UpdatePurchaseRequestDetailComment): Promise<any> {
    this.logger.debug(
      {
        function: "update",
        id,
    ...
    const existingComment = await this.prismaService.tb_purchase_request_detail_comment.findFirst({
      where: {
        id: id,
        deleted_at: null,
      },
    ...
    const comment = await this.prismaService.tb_purchase_request_detail_comment.update({
      where: { id: id },
      data: updateData,
    });
  ...
  async delete(id: string): Promise<any> {
    ...
    // Soft delete
    await this.prismaService.tb_purchase_request_detail_comment.update({
      where: { id: id },
      data: {
        deleted_at: new Date(),
