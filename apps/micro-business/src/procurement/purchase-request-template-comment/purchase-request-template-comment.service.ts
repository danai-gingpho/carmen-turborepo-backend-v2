...
import {
  CreatePurchaseRequestTemplateComment,
  UpdatePurchaseRequestTemplateComment,
  Attachment,
...
export class PurchaseRequestTemplateCommentService {
  ...
  private readonly logger: BackendLogger = new BackendLogger(PurchaseRequestTemplateCommentService.name);
  ...
  async findById(id: string): Promise<any> {
    ...
    const comment = await this.prismaService.tb_purchase_request_template_comment.findFirst({
      where: {
        id: id,
        deleted_at: null,
      },
   ...
   * @param purchaseRequestTemplateId - Purchase request ID / ID ของใบขอซื้อ
   * @param paginate - Pagination parameters / พารามิเตอร์การแบ่งหน้า
   * @returns Paginated list of comments with user profiles / รายการความคิดเห็นพร้อมโปรไฟล์ผู้ใช้ที่แบ่งหน้าแล้ว
   */
  async findAllByPurchaseRequestTemplateId(purchaseRequestTemplateId: string, paginate: IPaginate): Promise<any> {
    this.logger.debug(
      {
        function: "findAllByPurchaseRequestTemplateId",
    ...
    const whereQry = {
      ...q.where(),
      purchase_request_template_id: purchaseRequestTemplateId,
      deleted_at: null,
    };

    const comments = await this.prismaService.tb_purchase_request_template_comment.findMany({
      where: whereQry,
      orderBy: q.orderBy().length > 0 ? q.orderBy() : { created_at: "desc" },
      ...pagination,
    });

    const total = await this.prismaService.tb_purchase_request_template_comment.count({
      where: whereQry,
    });
   ...
   */
  async create(createDto: CreatePurchaseRequestTemplateComment): Promise<any> {
    this.logger.debug(
      {
        function: "create",
        createDto,
    ...
    const purchaseRequestTemplate = await this.prismaService.tb_purchase_request_template.findFirst({
      where: {
        id: createDto.purchase_request_template_id,
      },
    });

    if (!purchaseRequestTemplate) {
      return {
        response: {
          status: HttpStatus.NOT_FOUND,
    ...
    const comment = await this.prismaService.tb_purchase_request_template_comment.create({
      data: {
        purchase_request_template_id: createDto.purchase_request_template_id,
   ...
   */
  async update(id: string, updateDto: UpdatePurchaseRequestTemplateComment): Promise<any> {
    this.logger.debug(
      {
        function: "update",
        id,
    ...
    const existingComment = await this.prismaService.tb_purchase_request_template_comment.findFirst({
      where: {
        id: id,
        deleted_at: null,
      },
    ...
    const comment = await this.prismaService.tb_purchase_request_template_comment.update({
      where: { id: id },
      data: updateData,
    });
  ...
  async delete(id: string): Promise<any> {
    ...
    // Soft delete
    await this.prismaService.tb_purchase_request_template_comment.update({
      where: { id: id },
      data: {
        deleted_at: new Date(),
