...
export class PurchaseOrderCommentService {
  ...
  private readonly logger: BackendLogger = new BackendLogger(PurchaseOrderCommentService.name);
  ...
  async findById(id: string): Promise<any> {
    ...
    const comment = await this.prismaService.tb_purchase_order_comment.findFirst({
      where: {
        id: id,
        deleted_at: null,
      },
   ...
   * @param purchaseOrderId - Purchase request ID / ID ของใบขอซื้อ
   * @param paginate - Pagination parameters / พารามิเตอร์การแบ่งหน้า
   * @returns Paginated list of comments with user profiles / รายการความคิดเห็นพร้อมโปรไฟล์ผู้ใช้ที่แบ่งหน้าแล้ว
   */
  async findAllByPurchaseOrderId(purchaseOrderId: string, paginate: IPaginate): Promise<any> {
    this.logger.debug(
      {
        function: "findAllByPurchaseOrderId",
        purchaseOrderId,
        user_id: this.userId,
        tenant_id: this.bu_code,
        paginate,
    ...
    const comments = await this.prismaService.tb_purchase_order_comment.findMany({
      where: whereQry,
      orderBy: q.orderBy().length > 0 ? q.orderBy() : { created_at: "desc" },
      ...pagination,
    });

    const total = await this.prismaService.tb_purchase_order_comment.count({
      where: whereQry,
    });
   ...
   */
  async create(createDto: CreatePurchaseOrderComment): Promise<any> {
    this.logger.debug(
      {
        function: "create",
        createDto,
    ...
    const purchaseOrder = await this.prismaService.tb_purchase_order.findFirst({
      where: {
        id: createDto.purchase_order_id,
      },
    });
    ...
    const comment = await this.prismaService.tb_purchase_order_comment.create({
      data: {
        purchase_order_id: createDto.purchase_order_id,
   ...
   */
  async update(id: string, updateDto: UpdatePurchaseOrderComment): Promise<any> {
    this.logger.debug(
      {
        function: "update",
        id,
    ...
    const existingComment = await this.prismaService.tb_purchase_order_comment.findFirst({
      where: {
        id: id,
        deleted_at: null,
      },
    ...
    const comment = await this.prismaService.tb_purchase_order_comment.update({
      where: { id: id },
      data: updateData,
    });
  ...
  async delete(id: string): Promise<any> {
    ...
    // Soft delete
    await this.prismaService.tb_purchase_order_comment.update({
      where: { id: id },
      data: {
        deleted_at: new Date(),
