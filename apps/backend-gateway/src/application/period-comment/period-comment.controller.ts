...
import {
  UpdatePeriodCommentDto,
  UpdatePeriodCommentBodySchema,
  AddAttachmentsDto,
...
export class PeriodCommentController {
  ...
  @Get(':bu_code/period-comment/:period_id')
  @UseGuards(new AppIdGuard('periodComment.findAll'))
  @ApiVersionMinRequest()
  @ApiUserFilterQueries()
  @ApiOperation({
    summary: 'Get all comments for a period',
    operationId: 'findAllPeriodComments',
    responses: {
  ...
  async findAllByParentId(
    ...
    @Param('period_id', new ParseUUIDPipe({ version: '4' })) period_id: string,
    @Req() req: Request,
    ...
    return this.periodCommentService.findAllByParentId(
      period_id,
      user_id,
      bu_code,
      paginate,
  ...
  @ApiVersionMinRequest()
  @ApiOperation({
    summary: 'Update a period comment with attachment add/remove',
    operationId: 'updatePeriodComment',
    responses: {
  ...
  async update(
    ...
    const parsed = UpdatePeriodCommentBodySchema.safeParse(rawBody);
    if (!parsed.success) {
    ...
    const { user_id } = ExtractRequestHeader(req);
    return this.periodCommentService.update(
      id,
      {
  ...
  @ApiVersionMinRequest()
  @ApiOperation({
    summary: 'Delete a period comment',
    operationId: 'deletePeriodComment',
    responses: {
  ...
  @ApiVersionMinRequest()
  @ApiOperation({
    summary: 'Add attachments to a period comment',
    operationId: 'addAttachmentsToPeriodComment',
    responses: {
  ...
  async addAttachment(
    ...
    const { user_id } = ExtractRequestHeader(req);
    return this.periodCommentService.addAttachments(
      id,
      files,
      user_id,
      bu_code,
  ...
  @ApiVersionMinRequest()
  @ApiOperation({
    summary: 'Remove an attachment from a period comment',
    operationId: 'removeAttachmentFromPeriodComment',
    responses: {
  ...
  @Post(':bu_code/period-comment/:period_id')
  ...
  async createWithFiles(
    @Param('bu_code') bu_code: string,
    @Param('period_id', new ParseUUIDPipe({ version: '4' }))
    period_id: string,
    @UploadedFiles() files: Express.Multer.File[] = [],
    @Body() rawBody: Record<string, unknown>,
    @Req() req: Request,
