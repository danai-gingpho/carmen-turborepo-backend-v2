...
import {
  UpdatePricelistCommentDto,
  UpdatePricelistCommentBodySchema,
  AddAttachmentsDto,
...
export class PricelistCommentController {
  ...
  @Get(':bu_code/pricelist-comment/:pricelist_id')
  @UseGuards(new AppIdGuard('pricelistComment.findAll'))
  @ApiVersionMinRequest()
  @ApiUserFilterQueries()
  @ApiOperation({
    summary: 'Get all comments for a pricelist',
    operationId: 'findAllPricelistComments',
    responses: {
  ...
  async findAllByParentId(
    ...
    @Param('pricelist_id', new ParseUUIDPipe({ version: '4' })) pricelist_id: string,
    @Req() req: Request,
    ...
    return this.pricelistCommentService.findAllByParentId(
      pricelist_id,
      user_id,
      bu_code,
      paginate,
  ...
  @ApiVersionMinRequest()
  @ApiOperation({
    summary: 'Update a pricelist comment with attachment add/remove',
    operationId: 'updatePricelistComment',
    responses: {
  ...
  async update(
    ...
    const parsed = UpdatePricelistCommentBodySchema.safeParse(rawBody);
    if (!parsed.success) {
    ...
    const { user_id } = ExtractRequestHeader(req);
    return this.pricelistCommentService.update(
      id,
      {
  ...
  @ApiVersionMinRequest()
  @ApiOperation({
    summary: 'Delete a pricelist comment',
    operationId: 'deletePricelistComment',
    responses: {
  ...
  @ApiVersionMinRequest()
  @ApiOperation({
    summary: 'Add attachments to a pricelist comment',
    operationId: 'addAttachmentsToPricelistComment',
    responses: {
  ...
  async addAttachment(
    ...
    const { user_id } = ExtractRequestHeader(req);
    return this.pricelistCommentService.addAttachments(
      id,
      files,
      user_id,
      bu_code,
  ...
  @ApiVersionMinRequest()
  @ApiOperation({
    summary: 'Remove an attachment from a pricelist comment',
    operationId: 'removeAttachmentFromPricelistComment',
    responses: {
  ...
  @Post(':bu_code/pricelist-comment/:pricelist_id')
  ...
  async createWithFiles(
    @Param('bu_code') bu_code: string,
    @Param('pricelist_id', new ParseUUIDPipe({ version: '4' }))
    pricelist_id: string,
    @UploadedFiles() files: Express.Multer.File[] = [],
    @Body() rawBody: Record<string, unknown>,
    @Req() req: Request,
