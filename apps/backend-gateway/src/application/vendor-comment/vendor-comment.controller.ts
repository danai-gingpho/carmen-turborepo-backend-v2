...
import {
  UpdateVendorCommentDto,
  UpdateVendorCommentBodySchema,
  AddAttachmentsDto,
...
export class VendorCommentController {
  ...
  @Get(':bu_code/vendor-comment/:vendor_id')
  @UseGuards(new AppIdGuard('vendorComment.findAll'))
  @ApiVersionMinRequest()
  @ApiUserFilterQueries()
  @ApiOperation({
    summary: 'Get all comments for a vendor',
    operationId: 'findAllVendorComments',
    responses: {
  ...
  async findAllByParentId(
    ...
    return this.vendorCommentService.findAllByParentId(
      vendor_id,
      user_id,
      bu_code,
      paginate,
  ...
  @ApiVersionMinRequest()
  @ApiOperation({
    summary: 'Update a vendor comment with attachment add/remove',
    operationId: 'updateVendorComment',
    responses: {
  ...
  async update(
    ...
    const parsed = UpdateVendorCommentBodySchema.safeParse(rawBody);
    if (!parsed.success) {
    ...
    const { user_id } = ExtractRequestHeader(req);
    return this.vendorCommentService.update(
      id,
      {
  ...
  @ApiVersionMinRequest()
  @ApiOperation({
    summary: 'Add attachments to a vendor comment',
    operationId: 'addAttachmentsToVendorComment',
    responses: {
  ...
  async addAttachment(
    ...
    const { user_id } = ExtractRequestHeader(req);
    return this.vendorCommentService.addAttachments(
      id,
      files,
      user_id,
      bu_code,
  ...
  @ApiVersionMinRequest()
  @ApiOperation({
    summary: 'Remove an attachment from a vendor comment',
    operationId: 'removeAttachmentFromVendorComment',
    responses: {
  ...
  @Post(':bu_code/vendor-comment/:vendor_id')
  ...
  async createWithFiles(
    @Param('bu_code') bu_code: string,
    @Param('vendor_id', new ParseUUIDPipe({ version: '4' }))
    vendor_id: string,
    @UploadedFiles() files: Express.Multer.File[] = [],
    @Body() rawBody: Record<string, unknown>,
    @Req() req: Request,
