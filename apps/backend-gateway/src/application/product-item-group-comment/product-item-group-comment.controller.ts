...
import {
  UpdateProductItemGroupCommentDto,
  UpdateProductItemGroupCommentBodySchema,
  AddAttachmentsDto,
} from './dto/product-item-group-comment.dto';
...
export class ProductItemGroupCommentController {
  private readonly logger: BackendLogger = new BackendLogger(
    ProductItemGroupCommentController.name,
  );

  constructor(
    private readonly productItemGroupCommentService: ProductItemGroupCommentService,
  ) {}

  @Get(':bu_code/product-item-group-comment/:product_item_group_id')
  @UseGuards(new AppIdGuard('productItemGroupComment.findAll'))
  @ApiVersionMinRequest()
  @ApiUserFilterQueries()
  @ApiOperation({
    summary: 'Get all comments for a product-item-group',
    operationId: 'findAllProductItemGroupComments',
    responses: {
  ...
  async findAllByParentId(
    ...
    @Param('product_item_group_id', new ParseUUIDPipe({ version: '4' })) product_item_group_id: string,
    @Req() req: Request,
    ...
    return this.productItemGroupCommentService.findAllByParentId(
  ...
  @ApiVersionMinRequest()
  @ApiOperation({
    summary: 'Update a product-item-group comment with attachment add/remove',
    operationId: 'updateProductItemGroupComment',
    responses: {
  ...
  async update(
    ...
    const parsed = UpdateProductItemGroupCommentBodySchema.safeParse(rawBody);
    if (!parsed.success) {
    ...
    const { user_id } = ExtractRequestHeader(req);
    return this.productItemGroupCommentService.update(
      id,
      {
  ...
  @ApiVersionMinRequest()
  @ApiOperation({
    summary: 'Delete a product-item-group comment',
    operationId: 'deleteProductItemGroupComment',
    responses: {
  ...
  @ApiVersionMinRequest()
  @ApiOperation({
    summary: 'Add attachments to a product-item-group comment',
    operationId: 'addAttachmentsToProductItemGroupComment',
    responses: {
  ...
  async addAttachment(
    ...
    const { user_id } = ExtractRequestHeader(req);
    return this.productItemGroupCommentService.addAttachments(
      id,
      files,
      user_id,
      bu_code,
  ...
  @ApiVersionMinRequest()
  @ApiOperation({
    summary: 'Remove an attachment from a product-item-group comment',
    operationId: 'removeAttachmentFromProductItemGroupComment',
    responses: {
  ...
  @Post(':bu_code/product-item-group-comment/:product_item_group_id')
  ...
  async createWithFiles(
    @Param('bu_code') bu_code: string,
    @Param('product_item_group_id', new ParseUUIDPipe({ version: '4' }))
