...
import {
  UpdateTaxProfileCommentDto,
  UpdateTaxProfileCommentBodySchema,
  AddAttachmentsDto,
...
export class TaxProfileCommentController {
  ...
  @Get(':bu_code/tax-profile-comment/:tax_profile_id')
  @UseGuards(new AppIdGuard('taxProfileComment.findAll'))
  @ApiVersionMinRequest()
  @ApiUserFilterQueries()
  @ApiOperation({
    summary: 'Get all comments for a tax-profile',
    operationId: 'findAllTaxProfileComments',
    responses: {
  ...
  async findAllByParentId(
    ...
    @Param('tax_profile_id', new ParseUUIDPipe({ version: '4' })) tax_profile_id: string,
    @Req() req: Request,
    ...
    return this.taxProfileCommentService.findAllByParentId(
      tax_profile_id,
      user_id,
      bu_code,
      paginate,
  ...
  @ApiVersionMinRequest()
  @ApiOperation({
    summary: 'Update a tax-profile comment with attachment add/remove',
    operationId: 'updateTaxProfileComment',
    responses: {
  ...
  async update(
    ...
    const parsed = UpdateTaxProfileCommentBodySchema.safeParse(rawBody);
    if (!parsed.success) {
    ...
    const { user_id } = ExtractRequestHeader(req);
    return this.taxProfileCommentService.update(
      id,
      {
  ...
  @ApiVersionMinRequest()
  @ApiOperation({
    summary: 'Delete a tax-profile comment',
    operationId: 'deleteTaxProfileComment',
    responses: {
  ...
  @ApiVersionMinRequest()
  @ApiOperation({
    summary: 'Add attachments to a tax-profile comment',
    operationId: 'addAttachmentsToTaxProfileComment',
    responses: {
  ...
  async addAttachment(
    ...
    const { user_id } = ExtractRequestHeader(req);
    return this.taxProfileCommentService.addAttachments(
      id,
      files,
      user_id,
      bu_code,
  ...
  @ApiVersionMinRequest()
  @ApiOperation({
    summary: 'Remove an attachment from a tax-profile comment',
    operationId: 'removeAttachmentFromTaxProfileComment',
    responses: {
  ...
  @Post(':bu_code/tax-profile-comment/:tax_profile_id')
  ...
  async createWithFiles(
    @Param('bu_code') bu_code: string,
    @Param('tax_profile_id', new ParseUUIDPipe({ version: '4' }))
