...
import {
  UpdateDeliveryPointCommentDto,
  UpdateDeliveryPointCommentBodySchema,
  AddAttachmentsDto,
} from './dto/delivery-point-comment.dto';
...
export class DeliveryPointCommentController {
  private readonly logger: BackendLogger = new BackendLogger(
    DeliveryPointCommentController.name,
  );

  constructor(
    private readonly deliveryPointCommentService: DeliveryPointCommentService,
  ) {}

  @Get(':bu_code/delivery-point-comment/:delivery_point_id')
  @UseGuards(new AppIdGuard('deliveryPointComment.findAll'))
  @ApiVersionMinRequest()
  @ApiUserFilterQueries()
  @ApiOperation({
    summary: 'Get all comments for a delivery-point',
    operationId: 'findAllDeliveryPointComments',
    responses: {
  ...
  async findAllByParentId(
    ...
    @Param('delivery_point_id', new ParseUUIDPipe({ version: '4' })) delivery_point_id: string,
    @Req() req: Request,
    ...
    return this.deliveryPointCommentService.findAllByParentId(
      delivery_point_id,
      user_id,
      bu_code,
      paginate,
  ...
  @ApiVersionMinRequest()
  @ApiOperation({
    summary: 'Update a delivery-point comment with attachment add/remove',
    operationId: 'updateDeliveryPointComment',
    responses: {
  ...
  async update(
    ...
    const parsed = UpdateDeliveryPointCommentBodySchema.safeParse(rawBody);
    if (!parsed.success) {
    ...
    const { user_id } = ExtractRequestHeader(req);
    return this.deliveryPointCommentService.update(
      id,
      {
  ...
  @ApiVersionMinRequest()
  @ApiOperation({
    summary: 'Delete a delivery-point comment',
    operationId: 'deleteDeliveryPointComment',
    responses: {
  ...
  @ApiVersionMinRequest()
  @ApiOperation({
    summary: 'Add attachments to a delivery-point comment',
    operationId: 'addAttachmentsToDeliveryPointComment',
    responses: {
  ...
  async addAttachment(
    ...
    const { user_id } = ExtractRequestHeader(req);
    return this.deliveryPointCommentService.addAttachments(
      id,
      files,
      user_id,
      bu_code,
  ...
  @ApiVersionMinRequest()
  @ApiOperation({
    summary: 'Remove an attachment from a delivery-point comment',
    operationId: 'removeAttachmentFromDeliveryPointComment',
    responses: {
  ...
  @Post(':bu_code/delivery-point-comment/:delivery_point_id')
  ...
  async createWithFiles(
    @Param('bu_code') bu_code: string,
    @Param('delivery_point_id', new ParseUUIDPipe({ version: '4' }))
