...
import {
  UpdateLocationCommentDto,
  UpdateLocationCommentBodySchema,
  AddAttachmentsDto,
...
export class LocationCommentController {
  ...
  @Get(':bu_code/location-comment/:location_id')
  @UseGuards(new AppIdGuard('locationComment.findAll'))
  @ApiVersionMinRequest()
  @ApiUserFilterQueries()
  @ApiOperation({
    summary: 'Get all comments for a location',
    operationId: 'findAllLocationComments',
    responses: {
  ...
  async findAllByParentId(
    ...
    return this.locationCommentService.findAllByParentId(
      location_id,
      user_id,
      bu_code,
      paginate,
  ...
  @ApiVersionMinRequest()
  @ApiOperation({
    summary: 'Update a location comment with attachment add/remove',
    operationId: 'updateLocationComment',
    responses: {
  ...
  async update(
    ...
    const parsed = UpdateLocationCommentBodySchema.safeParse(rawBody);
    if (!parsed.success) {
    ...
    const { user_id } = ExtractRequestHeader(req);
    return this.locationCommentService.update(
      id,
      {
  ...
  @ApiVersionMinRequest()
  @ApiOperation({
    summary: 'Delete a location comment',
    operationId: 'deleteLocationComment',
    responses: {
  ...
  @ApiVersionMinRequest()
  @ApiOperation({
    summary: 'Add attachments to a location comment',
    operationId: 'addAttachmentsToLocationComment',
    responses: {
  ...
  async addAttachment(
    ...
    const { user_id } = ExtractRequestHeader(req);
    return this.locationCommentService.addAttachments(
      id,
      files,
      user_id,
      bu_code,
  ...
  @ApiVersionMinRequest()
  @ApiOperation({
    summary: 'Remove an attachment from a location comment',
    operationId: 'removeAttachmentFromLocationComment',
    responses: {
  ...
  @Post(':bu_code/location-comment/:location_id')
  ...
  async createWithFiles(
    @Param('bu_code') bu_code: string,
    @Param('location_id', new ParseUUIDPipe({ version: '4' }))
    location_id: string,
    @UploadedFiles() files: Express.Multer.File[] = [],
    @Body() rawBody: Record<string, unknown>,
    @Req() req: Request,
