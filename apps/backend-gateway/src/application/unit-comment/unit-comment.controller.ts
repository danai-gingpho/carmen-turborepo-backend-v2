...
export class UnitCommentController {
  ...
  @Get(':bu_code/unit-comment/:unit_id')
  @UseGuards(new AppIdGuard('unitComment.findAll'))
  @ApiVersionMinRequest()
  @ApiUserFilterQueries()
  @ApiOperation({
    summary: 'Get all comments for a unit',
    operationId: 'findAllUnitComments',
    responses: {
  ...
  async findAllByParentId(
    ...
    @Param('unit_id', new ParseUUIDPipe({ version: '4' })) unit_id: string,
    @Req() req: Request,
    ...
    return this.unitCommentService.findAllByParentId(
      unit_id,
      user_id,
      bu_code,
      paginate,
  ...
  @ApiVersionMinRequest()
  @ApiOperation({
    summary: 'Update a unit comment with attachment add/remove',
    operationId: 'updateUnitComment',
    responses: {
  ...
  async update(
    ...
    const parsed = UpdateUnitCommentBodySchema.safeParse(rawBody);
    if (!parsed.success) {
    ...
    const { user_id } = ExtractRequestHeader(req);
    return this.unitCommentService.update(
      id,
      {
  ...
  @ApiVersionMinRequest()
  @ApiOperation({
    summary: 'Delete a unit comment',
    operationId: 'deleteUnitComment',
    responses: {
  ...
  @ApiVersionMinRequest()
  @ApiOperation({
    summary: 'Add attachments to a unit comment',
    operationId: 'addAttachmentsToUnitComment',
    responses: {
  ...
  async addAttachment(
    ...
    const { user_id } = ExtractRequestHeader(req);
    return this.unitCommentService.addAttachments(
      id,
      files,
      user_id,
      bu_code,
  ...
  @ApiVersionMinRequest()
  @ApiOperation({
    summary: 'Remove an attachment from a unit comment',
    operationId: 'removeAttachmentFromUnitComment',
    responses: {
  ...
  @Post(':bu_code/unit-comment/:unit_id')
  ...
  async createWithFiles(
    @Param('bu_code') bu_code: string,
    @Param('unit_id', new ParseUUIDPipe({ version: '4' }))
    unit_id: string,
    @UploadedFiles() files: Express.Multer.File[] = [],
    @Body() rawBody: Record<string, unknown>,
    @Req() req: Request,
