...
import {
  UpdateExchangeRateCommentDto,
  UpdateExchangeRateCommentBodySchema,
  AddAttachmentsDto,
...
export class ExchangeRateCommentController {
  ...
  @Get(':bu_code/exchange-rate-comment/:exchange_rate_id')
  @UseGuards(new AppIdGuard('exchangeRateComment.findAll'))
  @ApiVersionMinRequest()
  @ApiUserFilterQueries()
  @ApiOperation({
    summary: 'Get all comments for a exchange-rate',
    operationId: 'findAllExchangeRateComments',
    responses: {
  ...
  async findAllByParentId(
    ...
    @Param('exchange_rate_id', new ParseUUIDPipe({ version: '4' })) exchange_rate_id: string,
    @Req() req: Request,
    ...
    return this.exchangeRateCommentService.findAllByParentId(
      exchange_rate_id,
      user_id,
      bu_code,
      paginate,
  ...
  @ApiVersionMinRequest()
  @ApiOperation({
    summary: 'Update a exchange-rate comment with attachment add/remove',
    operationId: 'updateExchangeRateComment',
    responses: {
  ...
  async update(
    ...
    const parsed = UpdateExchangeRateCommentBodySchema.safeParse(rawBody);
    if (!parsed.success) {
    ...
    const { user_id } = ExtractRequestHeader(req);
    return this.exchangeRateCommentService.update(
      id,
      {
  ...
  @ApiVersionMinRequest()
  @ApiOperation({
    summary: 'Delete a exchange-rate comment',
    operationId: 'deleteExchangeRateComment',
    responses: {
  ...
  @ApiVersionMinRequest()
  @ApiOperation({
    summary: 'Add attachments to a exchange-rate comment',
    operationId: 'addAttachmentsToExchangeRateComment',
    responses: {
  ...
  async addAttachment(
    ...
    const { user_id } = ExtractRequestHeader(req);
    return this.exchangeRateCommentService.addAttachments(
      id,
      files,
      user_id,
      bu_code,
  ...
  @ApiVersionMinRequest()
  @ApiOperation({
    summary: 'Remove an attachment from a exchange-rate comment',
    operationId: 'removeAttachmentFromExchangeRateComment',
    responses: {
  ...
  @Post(':bu_code/exchange-rate-comment/:exchange_rate_id')
  ...
  async createWithFiles(
    @Param('bu_code') bu_code: string,
    @Param('exchange_rate_id', new ParseUUIDPipe({ version: '4' }))
